# Specification: mkadrlik Hermes Qwen3.5 4B SFT v7 Qwen No-Think Prefill Completion

## Overview

Close the endpoint-gated `qwen-no-think-prefill` prompt/profile repair row for
`mkadrlik/Hermes-Qwen3.5-4B-SFT-v7` using the already cached GGUF artifact.
...
The candidate scored `0/3`. The prefill introduced visible `<think>` tags,
malformed tool envelopes, and a forbidden delete tool call, so raw-output
