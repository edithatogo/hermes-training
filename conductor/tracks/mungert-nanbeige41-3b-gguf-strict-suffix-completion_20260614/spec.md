# Specification: Mungert Nanbeige4.1 3B GGUF Strict-Suffix Completion

## Overview

Close the endpoint-gated `strict-suffix-copy-exact` prompt/profile repair row
for `Mungert/Nanbeige4.1-3B-GGUF` using the already cached GGUF artifact.
...
The candidate scored `1/3`. It passed only the simple lookup tool-call case,
...
- Remaining gap: prompt-only repair did not solve parallel calls or strict
