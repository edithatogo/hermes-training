...
- [x] Task: Conductor - Automated Review and Checkpoint 'Scope, Inventory, And Shared Harness' (Protocol in workflow.md)
    - [x] Validation: `./.venv/bin/python scripts/validate_readiness.py`, `python3 -m unittest discover -s tests`, and `git diff --check` passed on 2026-05-24.
...
- [x] Task: Conductor - Automated Review and Checkpoint 'LM Studio Proof For Existing Qwen3 Q4_K_M GGUF' (Protocol in workflow.md)
    - [x] Evidence remains runtime-only; benchmark output is recorded and no publication claim is made from this lane alone.
...
- [x] Task: Conductor - Automated Review and Checkpoint 'Hermes 4 14B Runtime Proof' (Protocol in workflow.md)
    - [x] Evidence: runtime smoke and endpoint benchmarks are recorded under `reports/runtime/` and `reports/benchmark/endpoint-tool-call/`.
...
- [ ] Task: Conductor - Automated Review and Checkpoint 'Qwen3.6 Runtime Proof' (Protocol in workflow.md)
    - [ ] Blocked until `/Volumes/PortableSSD/hermes-models/frontier-gguf/qwen3.6-35b-a3b-q4/Qwen3.6-35B-A3B-Q4_K_M.gguf` reaches `21166757888` bytes and passes `scripts/run_qwen36_q4_runtime_proof.sh`.
...
- [x] Task: Conductor - Automated Review and Checkpoint 'OpenAI Normalizing Proxy Route' (Protocol in workflow.md)
    - [x] Evidence: self-test and recorded proxy smoke passed; direct llama.cpp and MLX OpenAI-compatible endpoints are now also covered by runtime reports.
...
- [x] Task: Conductor - Automated Review and Checkpoint 'Deferred Ollama Retest After Runtime Upgrade' (Protocol in workflow.md)
    - [x] Evidence: gate remains intentionally blocked on unchanged Ollama `0.24.0`; no unstable rerun was attempted.
