...
6. If wiring `scripts/mem0_read.py` into Hermes, expose it as an explicit/cached memory-read tool first; p50 live CLI latency is currently `2.897s`, while repeated cache-hit reads round to `0.000s`. Keep caching opt-in with a short TTL and refresh after writes. Keep Qwen3 0.6B as a learned-reranker candidate only after prompt/metadata work fixes the isolated fixture recency miss.
