...
- The Qwen3.5 0.8B strict-suffix repair result is recorded as
  ...
  still failed exact tool-call parsing. The `empty-output-retry` repair also
  scored `0/3`. Stop prompt-only Qwen3.5 0.8B repairs; use a grammar/envelope
  repair or move local repair effort to the next queued candidate.
- MiniCPM5 1B MLX prompt-only repair is also exhausted locally:
  `strict-suffix-copy-exact`, `empty-output-retry`, and
  `minicpm-empty-tag-repair` each scored `0/3` on the strict BFCL pilot. Keep it
  at `completed-no-promotion` and do not repeat prompt-only variants.
- Qwen3.5 2B prompt-only repair is exhausted locally: strict suffix and
  empty-output retry scored `0/3`, and no-think scored `1/3` by passing only the
  invalid-tool refusal. Keep it at `completed-no-promotion`.
- Gemma E4B MLX prompt/normalizer repair is exhausted locally: raw strict suffix
  and Gemma native normalizer analysis both scored `0/3`. The normalizer did not
  clear strict Hermes parsing or no-extra-text gates.
- Granite 4.1 3B prompt/normalizer repair is exhausted locally: raw strict
  suffix and Granite native normalizer analysis both scored `1/3`, passing only
  invalid-tool refusal and failing exact tool-call parsing.
- Nanbeige 4.1 3B strict-suffix repair scored `0/3`, but the tool-call payloads
  were semantically correct and failed because `<think>` traces/extra refusal
  text violated strict no-extra-text scoring.
...
- The constrained-envelope repair plan is now tracked at
  ...
  its two tool-call cases produced exact Hermes calls but failed strict scoring
  on extra text. Treat the report as a planning gate only: no model is promoted
...
- The Qwen3 v4 PEFT scorecard backend selector is now generated and tracked at
  ...
  that the kernel was still running with zero recovered files. Result artifact
  recovery plus no-pending ingest remain outstanding.
- Modal now has both a fail-closed execution contract and a result-ingest gate:
  ...
  local `--write-result` path; `reports/cloud/qwen3-v4-peft-modal-result-ingest-20260614.md`
  remains `pending_artifacts` until a scored result is recovered.
...
2. Use Colab first for sanitized bounded benchmark or smoke jobs via `scripts/colab_dispatch.py`; only attempt Azure after `az login`, `scripts/azure_preflight.py --check-quota`, and explicit cost approval pass.
   ...
   explicit run approval; Kaggle kernel version 2 now needs artifact recovery
   and no-pending ingest validation before it can affect any benchmark claim.
...
5. The local prompt-only repair queue is exhausted, and the first Nanbeige
   ...
   as historical execution state, not as a reason to repeat local prompt-only
   variants. Endpoint-gated rows still require existing artifacts/endpoints; do
   not redownload or promote from this queue alone. Gemma 4 E4B now has
   adapter-analysis evidence only:
