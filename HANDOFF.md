...
5. Next local runtime proofs should prioritize a Gemma 4 E4B MLX or prompt-profile path rather than repeating raw E2B GGUF, plus endpoint-format smokes for already acquired LFM2.5 8B and MiniCPM5 1B only if they are needed as helper/extraction baselines. MiniCPM5-1B is already MLX-load/loglikelihood proven and should move only to prompt-format repair or helper/extraction comparison against Qwen3.5 0.8B/2B. LFM2.5 8B is already GGUF load/generation proven but JSON-blocked. Gemma 4 E2B is GGUF load-proven but empty-output blocked. Use larger Gemma 4, Hermes 4, Qwen3.6, LFM2-24B, NVIDIA Nemotron, and BitNet as comparison/specialist baselines before attempting more local fine-tunes. For BitNet specifically, the next step is a non-interactive prompt wrapper plus Hermes extraction/tool-call smoke, not another raw `-cnv` run. For North Mini Code, wait for `cohere2moe` support in llama.cpp/LM Studio or switch to a safetensors/Transformers path.
