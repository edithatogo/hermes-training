...
6. Run a live read-only mem0 wrapper smoke with `score_plus_created_at_rank_close_margin`; do not switch the default embedder yet.
7. Restore Ollama responsiveness, then run the same expanded embedding and derived-reranker comparison for `nomic-embed-text:latest`.
