...
- The read-only mem0 wrapper now exposes `score_plus_created_at_rank_close_margin`, but live wrapper smoke is blocked because local Ollama binds to `127.0.0.1:11434` and then times out on `ollama list` / `/api/version`. The hung server process and Homebrew Ollama service were stopped; no mem0 defaults were changed. See `reports/benchmark/mem0/mem0-margin-rerank-live-smoke-blocked-20260526.md`.
...
6. Restore Ollama responsiveness for `nomic-embed-text:latest` and `sam860/LFM2:2.6b`, then run the live read-only mem0 wrapper smoke with `score_plus_created_at_rank_close_margin`.
