...
- Live multi-result comparison is now covered by an isolated non-sensitive fixture store, not by the default mem0 store. The default store still returned singleton results for broad probes, but the fixture used `MEM0_CONFIG_PATH`, an output-local Qdrant path, and a unique collection. Close-margin passed this gate; Qwen3 did not. No defaults were changed. See `reports/benchmark/mem0/live-multiretrieval-readiness-20260526.md` and `reports/benchmark/mem0/mem0-live-fixture-qwen3-multiretrieval-rerank-20260526.md`.
...
6. Implement a guarded close-margin read-wrapper integration path for mem0, with rollback docs and UX latency checks. Keep Qwen3 0.6B as a learned-reranker candidate only after prompt/metadata work fixes the isolated fixture recency miss.
