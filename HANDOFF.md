...
- 2026-06-11 model radar refresh still found no official Qwen3.7 open-weight lane. The major new actionable local candidates are `LiquidAI/LFM2.5-8B-A1B` / `LiquidAI/LFM2.5-8B-A1B-GGUF` and Gemma 4 26B-A4B QAT packaging (`google/gemma-4-26B-A4B-it-qat-q4_0-gguf`, `unsloth/gemma-4-26B-A4B-it-qat-GGUF`). `Qwen/Qwen3-Coder-Next` is now the strongest Qwen hybrid/subquadratic coding-agent baseline, but it belongs in Azure/specialist runtime proof, not Mac-local fine-tune. See `reports/model-radar/current-release-scan-20260524.md`.
...
5. Next local runtime proofs should prioritize LFM2.5-8B-A1B and Gemma 4 QAT; use Hermes 4, Qwen3.6, and LFM2-24B as comparison baselines/teachers before attempting more local fine-tunes.
