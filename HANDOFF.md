...
- 2026-06-11 model radar refresh still found no official Qwen3.7 open-weight lane. The major new actionable local candidates are `LiquidAI/LFM2.5-8B-A1B` / `LiquidAI/LFM2.5-8B-A1B-GGUF`, Gemma 4 QAT packaging from E2B/E4B through 31B, and `openbmb/MiniCPM5-1B` official GGUF/MLX packages. NVIDIA Nemotron 3 Nano / Nemotron Nano and `microsoft/bitnet-b1.58-2B-4T-gguf` are now explicit specialist runtime lanes. `Qwen/Qwen3-Coder-Next` is the strongest Qwen hybrid/subquadratic coding-agent baseline, but it belongs in Azure/specialist runtime proof, not Mac-local fine-tune. See `reports/model-radar/current-release-scan-20260524.md`.
- Specialist runtime preflight is now explicit and no-download. KTransformers, LEAP, RWKV, BitNet, and Mamba/SSM lanes were inspected for local CLI/module availability and exact SSD artifacts; all remain blocked before smoke. Existing GGUF/MLX proofs do not count as specialist runtime proof. See `reports/runtime/specialist-runtime-preflight-20260526.md`.
...
5. Next local runtime proofs should prioritize LFM2.5-8B-A1B, Gemma 4 E4B/12B QAT, and MiniCPM5-1B; use larger Gemma 4, Hermes 4, Qwen3.6, LFM2-24B, NVIDIA Nemotron, and BitNet as comparison/specialist baselines before attempting more local fine-tunes.
