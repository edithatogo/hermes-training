...
- LFM2.5 1.2B Instruct smoke LoRA is now runtime-proven through `mlx_lm.server` using only SSD-cached artifacts. The server exposed the absolute snapshot path as the model ID, and the OpenAI-compatible smoke returned `{"ok": true}` in `378ms`; direct `lfm2/scripts/evaluate.py` remains blocked by missing `num_layers` adapter metadata. See `reports/runtime/lfm25-1.2b-instruct-smoke-mlx-proof-20260526.md`.
