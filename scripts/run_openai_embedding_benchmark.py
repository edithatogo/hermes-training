#!/usr/bin/env python3
"""Run an embedding retrieval benchmark against an OpenAI-compatible endpoint."""
...
def parse_embedding_response(data: dict[str, Any]) -> list[float]:
    ...
    raise ValueError("embedding response did not include data[0].embedding or embedding")


def openai_embed(base_url: str, model: str, text: str, timeout_s: float) -> tuple[list[float], float]:
    url = base_url.rstrip("/") + "/embeddings"
    started = time.time()
    response = requests.post(url, json={"model": model, "input": text}, timeout=timeout_s)
    latency_s = time.time() - started
    response.raise_for_status()
    return parse_embedding_response(response.json()), latency_s
...
def render_summary_markdown(summary: dict[str, Any], rows: list[dict[str, Any]]) -> str:
    lines = [
        f"# OpenAI-Compatible Embedding Benchmark: {summary['run_id']}",
        "",
...
def main() -> int:
    ...
    run_id = args.run_id or f"openai-embedding-{datetime.now(UTC).strftime('%Y%m%d-%H%M%S')}"
    ...
    for index, case in enumerate(suite, 1):
        ...
        query_embedding, latency_s = openai_embed(args.base_url, args.model, case["query"], args.timeout_s)
        latencies.append(latency_s)
        embedding_dims = len(query_embedding)

        ranked: list[dict[str, Any]] = []
        for doc in case["documents"]:
            doc_embedding, doc_latency_s = openai_embed(args.base_url, args.model, doc["text"], args.timeout_s)
    ...
    summary = {
        ...
        "endpoint_kind": "openai-compatible-embeddings",
        "cases": cases,
