#!/usr/bin/env python3
"""Validate the generated runtime-proof action queue is current."""
...
DEFAULT_STEM = ROOT / "reports" / "benchmark" / "coverage" / "runtime-proof-action-queue-20260613"
...
def main() -> int:
    ...
    if not failures:
        ...
        if not isinstance(rows, list) or not rows:
            failures.append(f"{display_path(args.json_report)} has no queue rows")
        if not created_at:
    ...
    if not failures:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                [
                    sys.executable,
                    "scripts/build_runtime_proof_action_queue.py",
                    "--output-stem",
                    str(output_stem),
                    "--created-at",
                    created_at,
    ...
    if failures:
        print("not ready: runtime-proof action queue")
        for item in failures:
            print(f"- {item}")
        return 1

    print("ready: runtime-proof action queue is current")
    return 0
