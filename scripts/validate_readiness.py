...
def check_shell_syntax(failures: list[str]) -> None:
    ...
    py_scripts = [
        ...
        ROOT / "scripts/validate_qwen3_v12_bfcl_parallel_repair_dataset.py",
        ...
        ROOT / "scripts/validate_qwen3_v11_bfcl_handler_reasoning_bridge_report.py",
        ...
        ROOT / "scripts/validate_qwen3_v12_bfcl_parallel_repair_run_report.py",
...
def check_qwen3_v12_bfcl_parallel_repair_dataset(failures: list[str]) -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "scripts/validate_qwen3_v12_bfcl_parallel_repair_dataset.py")],
        capture_output=True,
        text=True,
    )
    if result.returncode:
        fail(f"qwen3 v12 BFCL parallel repair dataset: {result.stdout.strip()} {result.stderr.strip()}".strip(), failures)
    else:
        ok("qwen3 v12 BFCL parallel repair dataset")
...
def check_bfcl_completion_suffix_diagnostic_report(failures: list[str]) -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "scripts/validate_bfcl_completion_suffix_diagnostic_report.py")],
        capture_output=True,
        text=True,
    )
    if result.returncode:
        fail(f"BFCL completion-suffix diagnostic report: {result.stdout.strip()} {result.stderr.strip()}".strip(), failures)
    else:
        ok("BFCL completion-suffix diagnostic report")


def check_bfcl_text_prefix_bridge_report(failures: list[str]) -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "scripts/validate_bfcl_text_prefix_bridge_report.py")],
        capture_output=True,
        text=True,
    )
    if result.returncode:
        fail(f"BFCL text-prefix bridge report: {result.stdout.strip()} {result.stderr.strip()}".strip(), failures)
    else:
...
def check_qwen3_v11_bfcl_handler_reasoning_bridge_report(failures: list[str]) -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "scripts/validate_qwen3_v11_bfcl_handler_reasoning_bridge_report.py")],
        capture_output=True,
        text=True,
    )
    if result.returncode:
        fail(
            f"qwen3 v11 BFCL handler reasoning bridge report: {result.stdout.strip()} {result.stderr.strip()}".strip(),
            failures,
        )
    else:
        ok("qwen3 v11 BFCL handler reasoning bridge report")
...
def check_qwen3_v12_bfcl_parallel_repair_run_report(failures: list[str]) -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "scripts/validate_qwen3_v12_bfcl_parallel_repair_run_report.py")],
        capture_output=True,
        text=True,
    )
    if result.returncode:
        fail(f"qwen3 v12 BFCL parallel repair-run report: {result.stdout.strip()} {result.stderr.strip()}".strip(), failures)
    else:
        ok("qwen3 v12 BFCL parallel repair-run report")
...
def main() -> int:
    ...
    check_qwen3_v12_bfcl_parallel_repair_dataset(failures)
    ...
    check_qwen3_v12_bfcl_parallel_repair_run_report(failures)
    ...
    check_qwen3_v11_bfcl_handler_reasoning_bridge_report(failures)
