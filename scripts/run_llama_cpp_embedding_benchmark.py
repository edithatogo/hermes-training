#!/usr/bin/env python3
"""Run an embedding retrieval benchmark against llama.cpp `llama-embedding`."""
...
def validate_suite(suite: list[Any], suite_path: Path) -> None:
    ...
    for index, case in enumerate(suite, 1):
        ...
        if sum(1 for doc in docs if isinstance(doc, dict) and bool(doc.get("relevant"))) < 1:
        ...
        for doc in docs:
            ...
            if doc_id in doc_ids:
                raise ValueError(f"{case_id}: duplicate document id {doc_id}")
            doc_ids.add(doc_id)
...
def extract_first_json_object(text: str) -> dict[str, Any]:
    ...
    for index, char in enumerate(text):
        ...
        if isinstance(value, dict):
            return value
    raise ValueError(f"llama-embedding output did not contain JSON: {text[:500]}")


def llama_embed(args: argparse.Namespace, text: str) -> tuple[list[float], float]:
    command = [
        str(args.llama_embedding_bin),
        "-m",
        str(args.model_path),
        "--ctx-size",
        str(args.ctx_size),
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"llama-embedding failed with {completed.returncode}:\n{completed.stdout}")
    data = extract_first_json_object(completed.stdout)
    rows = data.get("data")
    if not isinstance(rows, list) or not rows:
        raise ValueError("llama-embedding JSON did not include data rows")
    embedding = rows[0].get("embedding") if isinstance(rows[0], dict) else None
    if not isinstance(embedding, list) or not embedding:
        raise ValueError("llama-embedding JSON did not include a non-empty embedding")
...
def render_summary_markdown(summary: dict[str, Any], rows: list[dict[str, Any]]) -> str:
    lines = [
        f"# llama.cpp Embedding Benchmark: {summary['run_id']}",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--model", required=True, help="Display model id for reports")
    parser.add_argument("--model-path", type=Path, required=True)
    parser.add_argument("--llama-embedding-bin", type=Path, default=Path("/opt/homebrew/bin/llama-embedding"))
    ...
    run_id = args.run_id or f"llama-cpp-embedding-{datetime.now(UTC).strftime('%Y%m%d-%H%M%S')}"
    ...
    for index, case in enumerate(suite, 1):
        ...
        query_embedding, latency_s = llama_embed(args, case["query"])
        latencies.append(latency_s)
        embedding_dims = len(query_embedding)

        ranked: list[dict[str, Any]] = []
        for doc in case["documents"]:
            doc_embedding, doc_latency_s = llama_embed(args, doc["text"])
    ...
    summary = {
        ...
        "endpoint_kind": "llama-cpp-embedding",
        "llama_embedding_bin": str(args.llama_embedding_bin),
        "ctx_size": args.ctx_size,
        "pooling": args.pooling,
        "embd_normalize": args.embd_normalize,
        "cases": cases,
        "top1_accuracy": sum(1 for row in rows if row["top1_pass"]) / cases,
        "recall_at_3": sum(float(row["recall_at_3"]) for row in rows) / cases,
        "mrr": statistics.mean(float(row["reciprocal_rank"]) for row in rows),
        "ndcg_at_3": statistics.mean(float(row["ndcg_at_3"]) for row in rows),
        "embedding_dims": embedding_dims,
        "embed_latency_mean_s": statistics.mean(latencies) if latencies else 0.0,
