#!/usr/bin/env python3
"""Validate the Qwen3 v8 wrapper/copy/refusal repair dataset and config."""
...
DATA_DIR = ROOT / "gemma4/data/strict_tool_call/expanded_splits_v8_wrapper_copy_refusal_repair"
BASE_DIR = ROOT / "gemma4/data/strict_tool_call/expanded_splits_v7_safety_refusal_repair"
MATERIALIZER = ROOT / "gemma4/data/strict_tool_call/tools/materialize_safety_refusal_repair_splits_v8.py"
CONFIG = ROOT / "gemma4/scripts/train_config.qwen3-4b.strict-toolcall-v8-wrapper-copy-refusal-repair.yaml"
REPAIR_PREFIX = "exp-v8-"
REQUIRED_LANES = {
...
def validate_dataset(data_dir: Path = DATA_DIR) -> list[str]:
    ...
    if len(train) != len(base_train) + sum(REQUIRED_LANES.values()):
        failures.append("v8 train must equal v7 train plus the narrow v8 repair rows")
    lane_counts = Counter(str(row.get("repair_lane", "")) for row in repair_rows)
    if dict(lane_counts) != REQUIRED_LANES:
        failures.append(f"v8 repair lane counts must be {REQUIRED_LANES}, got {dict(lane_counts)}")
    overlap = sorted(HELDOUT_IDS & {str(row.get("id", "")) for row in train})
    if overlap:
        failures.append(f"v8 repair train must not include held-out ids: {overlap}")
    for row in repair_rows:
        ...
        if row.get("repair_lane") == "exact-free-text-argument-copying" and "exp-v8-copy-002" in str(row.get("id")):
            if "Patient asked: can we move review to Friday?" not in final:
                failures.append(f"{row.get('id')}: exact note text was not copied into the tool argument")
...
def validate_config() -> list[str]:
    ...
    if cfg.get("model") != "Qwen/Qwen3-4B-MLX-4bit":
        failures.append("v8 config must keep the Qwen3 4B MLX base")
    if cfg.get("data") != "data/strict_tool_call/expanded_splits_v8_wrapper_copy_refusal_repair":
        failures.append("v8 config must point at the v8 repair dataset")
    if cfg.get("adapter_path") != "experiments/qwen3-4b-strict-toolcall-v8-wrapper-copy-refusal-repair/lora_adapter":
        failures.append("v8 config must write to the v8 adapter path")
    if "v7" in str(cfg.get("adapter_path", "")):
        failures.append("v8 config must not overwrite v7 adapter artifacts")
    if int(cfg.get("iters", 0)) < 120:
        failures.append("v8 config should keep enough iterations for a bounded repair run")
    if float(cfg.get("learning_rate", 0.0)) > 2.5e-5:
        failures.append("v8 config should use a conservative repair learning rate")
    return failures
...
def main() -> int:
    ...
    if not args.skip_regeneration_check and not failures:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_data = Path(tmp) / "expanded_splits_v8_wrapper_copy_refusal_repair"
            subprocess.run(
    ...
    if failures:
        print("not ready: qwen3 v8 repair dataset")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("ready: qwen3 v8 repair dataset")
    return 0
