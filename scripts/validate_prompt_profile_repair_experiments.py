#!/usr/bin/env python3
"""Validate prompt/profile repair experiment reports are current and strict."""
...
DEFAULT_STEM = ROOT / "reports/benchmark/coverage/prompt-profile-repair-experiments-20260614"
DEFAULT_JSON = DEFAULT_STEM.with_suffix(".json")
DEFAULT_MD = DEFAULT_STEM.with_suffix(".md")
MIN_EXPECTED_EXPERIMENTS = 20
...
def main() -> int:
    ...
    if not failures:
        ...
        if not isinstance(experiments, list) or len(experiments) < MIN_EXPECTED_EXPERIMENTS:
            failures.append(f"{display_path(args.json_report)} has too few experiments")
        if not created_at:
        ...
        for experiment in experiments if isinstance(experiments, list) else []:
            label = f"{experiment.get('candidate', '<unknown>')}:{experiment.get('variant', '<unknown>')}"
            ...
            if not experiment.get("goal"):
                failures.append(f"{label} has no experiment goal")
            if experiment.get("variant", "").endswith("-analysis") and experiment.get("raw_output_promotion_allowed"):
                failures.append(f"{label} allows raw promotion for analysis-only normalizer")
            if experiment.get("runner") == "endpoint" and "--score-normalizer" in command:
                failures.append(f"{label} uses unsupported endpoint score-normalizer flag")
    ...
    if not failures:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                [
                    sys.executable,
                    "scripts/build_prompt_profile_repair_experiments.py",
                    "--output-stem",
                    str(output_stem),
                    "--created-at",
                    created_at,
    ...
    if failures:
        print("not ready: prompt/profile repair experiments")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("ready: prompt/profile repair experiments are current")
    return 0
