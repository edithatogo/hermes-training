...
- [Support lane surface refresh](./reports/model-radar/support-lane-surface-refresh-current-release-scan-20260612.md)
...
2026-06-12 refresh: the shortlist evidence now includes fresh strict MLX server and local pilot notes for `Qwen/Qwen3.5-0.8B`, `Qwen/Qwen3.5-2B`, and `openbmb/MiniCPM5-1B-MLX`. All three remain helper/extraction candidates rather than promotion targets because the BFCL-style pilot still fails strict Hermes formatting. Keep `Qwen/Qwen3-4B-MLX-4bit` as the main adapter target, and treat `Hermes-4.3-36B`, `DJLougen/Harmonic-9B`, `mkadrlik/Hermes-Qwen3.5-9B-SFT-v7`, `Qwen/Qwen3.6-35B-A3B`, `Qwen/Qwen3.6-27B`, `google/gemma-4-12B-it`, `google/gemma-4-31B-it`, `openbmb/MiniCPM-o-4_5`, `openbmb/MiniCPM-V-4.6`, `Qwen/Qwen3-Coder-Next`, `Qwen/Qwen3-Embedding-4B`, `Qwen/Qwen3-Reranker-4B`, `Qwen/Qwen3-VL-Embedding-2B`, `Qwen/Qwen3-VL-Reranker-8B`, `jinaai/jina-embeddings-v5-omni-small`, `nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16`, and `nvidia/NVIDIA-Nemotron-3-Nano-4B-GGUF` as runtime/helper or teacher lanes behind the same strict proof sequence.
...
mem0 and embedding are now crystallized too: `BAAI/bge-m3` remains the current retrieval baseline, `lmstudio-community/embeddinggemma-300m-qat-GGUF` is the strongest 768-dim Mac-local challenger, `jinaai/jina-embeddings-v5-omni-small` and `jinaai/jina-embeddings-v5-omni-small-text-matching-mlx` remain alternate multimodal lanes, and `Qwen/Qwen3-Embedding-4B` plus `Qwen/Qwen3-Reranker-4B` are follow-on quality-ceiling candidates.
2026-06-13 refresh: EmbeddingGemma GGUF passed both direct `llama-embedding` and `llama-server` OpenAI-compatible differentiation runs at top-1 1.000 / recall@3 1.000. The first live output-local mem0 fixture reached top-1 0.909 / recall@3 1.000, captured fixture replay diagnosed the remaining miss, and the fresh resilient-proxy live fixture reached top-1 1.000 / recall@3 1.000 for both raw vector and `query_terms_guarded`. The `embeddinggemma-proxy` read mode now exposes this as an explicit opt-in path with `MEM0_CONFIG_PATH` / `--mem0-config-path`. The copied live-store replay over the current default_user/codex sample reached default-top recall 1.000 but top-1 match 0.200, and replay-only vector, query_terms_guarded, score_plus_created_at_rank, and close-margin reranking did not improve it. Keep EmbeddingGemma non-default until ranking behavior improves or a deliberate migration decision accepts the changed ordering.
...
The latest support search also surfaced `deepseek-ai/DeepSeek-V4-Pro`,
`nvidia/LocateAnything-3B`, and `bosonai/higgs-audio-v3-tts-4b` as fresh
teacher/helper lanes.
