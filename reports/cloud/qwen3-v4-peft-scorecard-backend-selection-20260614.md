...
| 7 | `kaggle` | `completed-failed-needs-kaggle-runner-fix` | -75 | Kaggle kernel version 5 completed without scores; the recovered summary is blocked, and this P100 path now needs a runner/runtime change or a different backend. Live Kaggle ingest failed after a completed kernel run; do not retry unchanged P100/CUDA path. |
